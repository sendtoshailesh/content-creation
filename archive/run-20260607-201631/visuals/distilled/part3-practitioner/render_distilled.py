...
def render_medium_hero(tokens):
    ...
    ax.text(W / 2, H * 0.72, "~450x",
            ha='center', va='center', fontsize=fs(96),
            fontfamily=FONT, fontweight='bold', color='#ffffff')

    ax.text(W / 2, H * 0.50, "per-request cost spread\nin GitHub AI Credits (Jun 1, 2026)",
    ...
    # Three stat callouts
    stats = [
        ("~$0.001", "Lightweight chat"),
        ("~$0.04", "Versatile review"),
        ("~$0.45", "Powerful agent"),
    ]
...
def render_medium_inline_01(tokens):
    ...
    # Bottom note
    ax.text(W / 2, H * 0.08, "Route lightweight tasks to Lightweight models -> ~86% savings with zero quality loss",
            ha='center', va='center', fontsize=fs(12),
            fontfamily=FONT, fontweight='bold', color=tokens['ACCENT'])

    ax.text(W / 2, H * 0.03, "Source: Apple ML Research, 2025; GitHub Docs (models and pricing), 2026 | Subject to change",
...
def render_medium_inline_02(tokens):
    ...
    ax.text(W / 2, H * 0.93, "Daily Cost Math: ~$4.07 (vs ~$30 If All Powerful) -- ~86% Savings",
            ha='center', va='center', fontsize=fs(16),
            fontfamily=FONT, fontweight='bold', color=tokens['TEXT'])

    # Stacked bar breakdown (per-day USD cost on a 100-request mix)
    # (label, count, $/request, color)
    categories = [
        ("Lightweight\n65 req", 65, 0.001, tokens['SUCCESS']),
        ("Versatile\n25 req", 25, 0.04, tokens['ACCENT']),
        ("Powerful\n10 req", 10, 0.30, tokens['WARN']),
    ]
    ...
    y_base = H * 0.22
    max_bar_h = H * 0.44
    max_cost = max(c * p for _, c, p, _ in categories)

    for (label, count, price, col), x in zip(categories, x_positions):
        daily = count * price
        bar_h = max(12, int(daily / max_cost * max_bar_h))
        rect = mpatches.FancyBboxPatch((x - bar_w / 2, y_base), bar_w, bar_h,
                                        boxstyle='square,pad=0',
                                        facecolor=col, linewidth=0, alpha=0.8)
        ax.add_patch(rect)

        # Per-request cost label on bar
        ax.text(x, y_base + bar_h + 20, f"@ ${price:.3f}/req",
                ha='center', va='bottom', fontsize=fs(13),
                fontfamily=FONT, fontweight='bold', color=col)

        # Daily total — inside bar if tall enough, else above bar
        if bar_h >= 50:
            ax.text(x, y_base + bar_h / 2, f"= ${daily:.2f}/day",
                    ha='center', va='center', fontsize=fs(11),
                    fontfamily=FONT, fontweight='bold', color='#ffffff')
        else:
            ax.text(x, y_base + bar_h + 55, f"= ${daily:.2f}/day",
    ...
    # Result box
    rect = mpatches.FancyBboxPatch((W * 0.20, H * 0.76), W * 0.60, H * 0.10,
                                    boxstyle='round,pad=6',
                                    facecolor=tokens['BLUE_BG'],
                                    edgecolor=tokens['ACCENT'], linewidth=2)
    ax.add_patch(rect)
    ax.text(W / 2, H * 0.81,
            "$0.07 + $1.00 + $3.00 = ~$4.07/day  ->  ~86% savings vs all-Powerful",
            ha='center', va='center', fontsize=fs(14),
            fontfamily=FONT, fontweight='bold', color=tokens['ACCENT'])

    ax.text(W / 2, H * 0.04,
            "RouteLLM: 95% flagship-quality at 75% cost reduction (LMSYS, 2024 -- flagship = GPT-4 baseline)",
            ha='center', va='center', fontsize=fs(9),
            fontfamily=FONT, color=tokens['MUTED'], style='italic')
