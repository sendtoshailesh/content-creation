...
**Visual hook note:** Slide 1 splits nouns (gym + equipment) from verbs (rep-scheme + coach) — the hook narrates that split.
...
▸ Böckeler's cleanest definition: a harness is "everything except the model"
...
Mature production setups are both at once: Stripe's "blueprints" and Anthropic's "Dynamic Workflows" are each a harness PLUS an explicit, code-defined loop with verification and stop logic.
...
▸ Böckeler: a harness is 𝘦𝘷𝘦𝘳𝘺𝘵𝘩𝘪𝘯𝘨 𝘦𝘹𝘤𝘦𝘱𝘵 𝘵𝘩𝘦 𝘮𝘰𝘥𝘦𝘭
...
Mature setups are both at once: Stripe's "blueprints" and Anthropic's "Dynamic Workflows" = a harness 𝗽𝗹𝘂𝘀 an explicit, code-defined loop.
...
▸ SWE-bench Verified: 12.47% (Mar 2024) → 76.8% (Feb 2026) — roughly 6x on a FIXED harness, model swapped underneath
...
▸ SWE-bench Verified: 𝟭𝟮.𝟰𝟳% → 𝟳𝟲.𝟴% — ~6x on a 𝗳𝗶𝘅𝗲𝗱 harness, model swapped underneath
