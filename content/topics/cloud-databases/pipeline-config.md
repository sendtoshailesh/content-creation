# Pipeline Config — Cloud databases

> Topic-scoped pipeline. The orchestrator reads this when run with topic `cloud-databases`
> (e.g. `/topic-pipeline cloud-databases`). Outputs land in this workspace
> (`content/topics/cloud-databases/`). Persistent files: `feed-sources.md`, `idea-queue.md`.
...
| **Topic slug** | `cloud-databases` |
| **Output path** | `content/topics/cloud-databases/` |
