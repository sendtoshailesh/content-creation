# Pipeline Config — PostgreSQL

> Topic-scoped pipeline. The orchestrator reads this when run with topic `postgresql`
> (e.g. `/topic-pipeline postgresql`). Outputs land in this workspace
> (`content/topics/postgresql/`). Persistent files: `feed-sources.md`, `idea-queue.md`.
...
| **Status** | `not-started` |
| **Topic** | PostgreSQL |
| **Topic slug** | `postgresql` |
| **Output path** | `content/topics/postgresql/` |
